...
$sql1="SELECT pickuptime FROM `request` where `donoremailid` = '$email'";
...
while ($i < $count)
{ 
	$sql1="SELECT recepientname FROM `request` where `donoremailid` = '$email' AND `pickuptime` = '$array3[$i]'";
	$result1 = mysqli_query($conn, $sql1);
...
while ($i < $count)
{ 
	$sql1="SELECT typefood FROM `request` where `donoremailid` = '$email' AND `pickuptime` = '$array3[$i]'";
	$result1 = mysqli_query($conn, $sql1);
...
while($k < $count)
{
    $list[] = array('volname' => $array6[$k], 'time' => $array3[$k],'recname'=>$array9[$k],'typefood'=>$array7[$k]);
    $k++;
}
